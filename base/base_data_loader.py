...
class NLPDataLoader():
    def __init__(self, dataset,split_ratio=None, batch_size=64, sort_key=None, device=None,
        ...
        self.word_embedding = word_embedding_path
    ...
    def get_iterators(self):
        ...
        if not self.split_ratio:
            ...
            if self.use_pretrained_word_embedding:
                ...
                self.dataset.TEXT.build_vocab(self.dataset, max_size=self.vocab_size, vectors=vectors,
                                              unk_init=torch.Tensor.normal_)
                self.dataset.LABEL.build_vocab(self.dataset)
                print('label types:{}'.format(self.dataset.LABEL.vocab.stoi))
            else:
                self.dataset.TEXT.build_vocab(self.dataset, max_size=self.vocab_size)  # 不使用预训练的词向量
                self.dataset.LABEL.build_vocab(self.dataset)
                print('label types:{}'.format(self.dataset.LABEL.vocab.stoi))
            ...
            if self.use_pretrained_word_embedding:
                vectors = Vectors(name=self.word_embedding_path)  # 使用预训练的词向量
                self.dataset.TEXT.build_vocab(train_data, max_size=self.vocab_size, vectors=vectors,
                                              unk_init=torch.Tensor.normal_)
                self.dataset.LABEL.build_vocab(train_data)
                print('label types:{}'.format(self.dataset.LABEL.vocab.stoi))
            else:
                self.dataset.TEXT.build_vocab(train_data, max_size=self.vocab_size)  # 不使用预训练的词向量
                self.dataset.LABEL.build_vocab(train_data)
                print('label types:{}'.format(self.dataset.LABEL.vocab.stoi))
...
class NLPDataset(Dataset):
    def __init__(self, data, text_field, label_field, test=False,stop_words_path=None, batch_first=False
                 , include_lengths=False,use_bert_embedding=False,bert_embedding_path=None):
        if stop_words_path:
        ...
        self.TEXT = Field(sequential=True,stop_words=stop_words, tokenize=tokenizer, batch_first=batch_first,
