...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            # preds = preds.squeeze().transpose(1,2)
            preds = preds.squeeze()
            ...
            # preds = torch.argmax(preds.transpose(1,2),dim=-1)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(self.valid_data_loader):
                ...
                if 'cuda' == self.device.type:
                    input_ids = input_ids.cuda()
                    if attention_masks is not None: # transformer
                        attention_masks = attention_masks.cuda()
                    else:
                        attention_masks = text_lengths # 非transformer
                    text_lengths = text_lengths.cuda()
                    labels = labels.cuda()
                preds, embedding = self.model(input_ids, attention_masks, text_lengths)
                preds = preds.squeeze()
    ...
    def _inference_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(self.test_data_loader):
                ...
                # preds = preds.squeeze().transpose(1, 2)
                preds = preds.squeeze()
                loss = self.criterion[0](preds, labels.unsqueeze(dim=-1))
